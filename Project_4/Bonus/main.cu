...
#define VOLUME_SIZE 1085440 //4096(super block)+32768(FCB)+1048576(Contents of files)
...
__device__ __managed__ uchar directory_control_memory[MAX_FILE_NUM*DFC_SIZE];
...
__global__ void mykernel(uchar *input, uchar *output) {
  ...
  fs_init(&fs, volume, SUPERBLOCK_SIZE, FCB_SIZE, FCB_ENTRIES, 
			...
			MAX_FILE_NUM, MAX_FILE_SIZE, FILE_BASE_ADDRESS, directory_control_memory, 0);
